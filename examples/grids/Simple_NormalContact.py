...
def addPlotData(ids):
	F = []
	for i in ids:
		F.append(O.forces.f(i).norm())
	maxT = 0
	for inter in O.interactions:
		if isinstance(inter.phys,CohFrictPhys):
			if inter.isReal:
				T = inter.phys.normalForce.norm()
				if maxT<T: maxT = T
			else:
				O.pause()
				print 'membrane broke...'
	plot.addData(i = O.iter, F1 = F[0], F2 = F[1], F3 = F[2], maxT = maxT)
...
O.engines=[
	...
	PyRunner(command='addPlotData(iIds)',iterPeriod=50000,label='plotter'),
]
...
# block rotation DOFs of membrane nodes
for i in mIds:
...
for i in range(3):
	iIds.append(O.bodies.append(
		gridNode(start+Vector3(dL*i,(i-1)**2*rGrid,0),rGrid,wire=True,fixed=True,material='iMat',color=color)))	
...
# fix both ends
for i in [mIds[0],mIds[-1]]:
	O.bodies[i].state.blockedDOFs = 'xyzXYZ'
# apply constant velocity on interface gridConnections
for i in iIds:
	O.bodies[i].state.vel = Vector3(0,-1e-3,0)
	
# run with plot
plot.plots = {'i':('F1','F2','F3',None,'maxT')}
O.dt = PWaveTimeStep()
O.saveTmp()
O.run()
