...
def addPlotData(mEnd,ids):
	e = O.bodies[mEnd].state.displ()[0]/O.bodies[mEnd].state.refPos[0]
	iF = Vector3(0,0,0,)
	for i in ids:
		iF += O.forces.f(i)
		iF += O.forces.permF(i)
	iFriction = degrees(atan(abs(iF[0]/iF[1])))
	avgT = 0
	num = 0
	for inter in O.interactions:
		if isinstance(inter.phys,CohFrictPhys):
			if inter.isReal:
				avgT += inter.phys.normalForce.norm()
				num+= 1
	avgT /= num
	plot.addData(e = e, iFriction = iFriction, avgT = avgT)
...
O.engines=[
	...
	PyRunner(command='addPlotData(mIds[-1],iIds)',iterPeriod=10000,label='plotter',dead=True),
]
...
for i in range(4*nL+1):
	mIds.append(O.bodies.append(
		gridNode(start+Vector3(dL*i,0,0),rGrid,wire=True,fixed=False,material='mMat',color=[1,1,0])))
# create connection between membrane nodes
for i,j in zip( mIds[:-1], mIds[1:]):
	O.bodies.append(gridConnection(i,j,rGrid,wire=True,material='m2iMat',color=[1,1,0]))
# block rotational and y-direction translational DOFs of membrane nodes
for i in mIds:
	O.bodies[i].state.blockedDOFs = 'yXYZ'
	
# create interface nodes
iIds = []
dL = 2.*rGrid*nL
start = Vector3(dL,2*rGrid,0)
for i in range(3):
	iIds.append(O.bodies.append(
		gridNode(start+Vector3(dL*i,0,0),rGrid,wire=True,fixed=False,material='iMat',color=color)))	
# create connection between interface nodes
for i,j in zip( iIds[:-1], iIds[1:]):
	O.bodies.append(gridConnection(i,j,rGrid,wire=True,material='m2iMat',color=color))
for i in iIds:
	O.bodies[i].state.blockedDOFs = 'xzXYZ'
				
## set boundary condition
# fix left ends
O.bodies[mIds[0]].state.blockedDOFs = 'xyzXYZ'
# apply constant force on interface gridConnections
O.dt = PWaveTimeStep()
for i in iIds:
	O.forces.addF(i,Vector3(0,-1e3,0),True)
while 1:
	O.run(5000)	
	if np.mean([O.bodies[i].state.vel.norm() for i in iIds]) < 1e-10:
		break

# apply constant velocity on interface gridConnections
for i in iIds:
	O.bodies[i].state.vel = Vector3(1e-3,0,0)
plotter.dead = False
O.saveTmp()

# run with plot
plot.plots = {'e':('iFriction',None,'avgT')}
rr = qt.Renderer()
O.run()
